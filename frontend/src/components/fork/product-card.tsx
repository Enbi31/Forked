...
export default function ProductCard({ product, index, onSelect }: ProductCardProps) {
  return (
    ...
    >
      <span
        className={`self-start px-3 py-1 rounded-full text-xs font-semibold text-white ${product.badgeColor}`}
      >
        {product.badgeLabel}
      </span>
      ...
      <div className="mt-4 p-3 bg-zinc-800/50 rounded-lg">
        <div className="flex items-start gap-2 text-sm text-zinc-400">
          ...
          <span>
            <span className="text-zinc-300 font-medium">The Trade-off: </span>
            {product.tradeoff}
          </span>
        </div>
      </div>
